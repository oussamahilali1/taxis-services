...
function renderHead({ title, description, canonical, image = site.ogImage, noindex = false, schemas = [] }) {
...
function renderLayout({ pageKey, title, description, canonical, bodyClass = '', image, noindex = false, schemas = [], mainContent }) {
  return `<!DOCTYPE html>
<html lang="${site.lang}">
<head>
${renderHead({ title, description, canonical, image, noindex, schemas })}
</head>
...
function renderBookingForm({ formId, sourceLabel, selectedService = '' }) {
  return `<form class="booking-form" id="${formId}" method="POST" action="/api/bookings" data-api-form="booking" data-form-variant="full" data-requires-contact="true">
    ...
    <div class="form-row">
      ...
      <div class="form-group">
        <label for="${formId}-passengers">Passagers</label>
        <select id="${formId}-passengers" name="passengers">
    ...
    <div class="form-row">
      ...
      <div class="form-group">
        <label for="${formId}-phone">Téléphone</label>
        <input id="${formId}-phone" type="tel" name="phone" placeholder="+32 470 12 34 56" data-contact-field>
      </div>
    </div>
    <div class="form-row">
      <div class="form-group">
    ...
    <div class="form-row">
      <div class="form-group full-width">
        <label for="${formId}-dropoff">Adresse de destination</label>
        <input id="${formId}-dropoff" type="text" name="dropoff" placeholder="Adresse d’arrivée ou lieu à desservir">
      </div>
    </div>
    <div class="form-row">
      <div class="form-group">
        <label for="${formId}-datetime">Date et heure souhaitées</label>
        <input id="${formId}-datetime" type="datetime-local" name="datetime">
      </div>
      <div class="form-group">
...
function renderHomeZonesSection() {
  return `<section id="zones" class="section areas">
    <div class="container">
      <div class="home-areas-inner">
        <div class="home-areas-content" data-animate="left">
          <div class="label divider-left">Zones desservies</div>
          <h2 class="heading-xl">Une base locale,<br><span class="text-gold">des trajets dans toute la région</span></h2>
          <p class="text-muted" style="margin-top: 20px;">
            Depuis Anderlues, nous préparons les trajets dans la région du Centre, vers Charleroi, les aéroports, les gares et d’autres destinations en Belgique selon la demande.
          </p>
          <div class="elegant-city-list">
            ${areas
              .map((area) => `<div class="elegant-city-item">
                <div class="elegant-city-icon">${icon('pin')}</div>
                <div class="elegant-city-text">
                  <h4>${area.title}</h4>
                  <p>${area.text}</p>
                </div>
              </div>`)
        ...
        <div class="home-areas-img" data-animate="right">
          <img src="assets/img/ex.jpg" alt="Taxi sur la route en Belgique" width="1024" height="683" loading="lazy">
        </div>
      </div>
    </div>
  </section>`;
  ...
  return renderLayout({
    ...
    <section id="zones" class="section services" style="background: var(--bg-alt);">
      <div class="container">
        <div class="services-header">
          <div class="label">Zones desservies</div>
          <h2 class="heading-xl">Au départ d’Anderlues<br><span class="text-gold">vers la région et la Belgique</span></h2>
        </div>
        <div class="services-grid">
          ${renderZonesCards()}
        </div>
